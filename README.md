...
To install:
  ``` bash
curl -sSL https://raw.githubusercontent.com/nathanvella743/flatstow/main/main.py -o ~/.local/bin/flatstow && chmod +x ~/.local/bin/flatstow && echo -e "\n Flatstow installed to ~/.local/bin/\n  Warn: If 'flatstow' command is not found open a new terminal window or ensure ~/.local/bin/ is in your PATH."
```
